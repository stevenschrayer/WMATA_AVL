...
if os.getlogin() == "WylieTimmerman":
    ...
    path_working = (
        r"C:\Users\WylieTimmerman\Documents\projects_local\WMATA_AVL_datamart"
    )
...
for analysis_route in analysis_routes:
    ...
    rawnav_agged_route_ti_basic_decomp = (
        rawnav_nofirstdoor.groupby(
            ["filename", "index_run_start", "id", "basic_decomp_ext"],
            sort=False,
            as_index=False,
        )
        .agg(
        ...
        .pivot(
            index=["filename", "index_run_start", "id"],
        ...
        .assign(
            ...
            # doors
            tot_nonstoprel_secs=lambda x: x.accel_nodoors_secs
    ...
    rawnav_ti_info = rawnav_nofirstdoor.drop_duplicates(
        subset=["route_pattern", "year", "filename", "index_run_start"]
    ).filter(["route_pattern", "year", "filename", "index_run_start"], axis="columns")

    rawnav_agged_route_ti = (
        rawnav_agged_route_ti_speed.merge(
            rawnav_agged_route_ti_basic_decomp,
            left_index=True,
            right_index=True,
            how="outer",
    ...
    rawnav_agged_route = (
        rawnav_agged_route_ti.groupby(["route_pattern", "year", "id"], sort=False)
        .agg(["mean"])
...
valid_shapes = gpd.read_file(
    os.path.join(path_sp, "data", "01-Interim", "valid_pattern_ids_hi.geojson")
).drop(["geometry"], axis="columns")

# re-add a few columns, join in flags for 'valid' segments
rawnav_agged_out = (
    rawnav_agged.assign(
        route=lambda x: x.route_pattern.str.strip().str.extract(
            pat="^([0-9A-Za-z]{2,3})[0-9]{2}$", expand=False
        ),
        pattern=lambda x: x.route_pattern.str.strip()
        .str.extract(pat="^[0-9A-Za-z]{2,3}([0-9]{2})$", expand=False)
        .astype(int),
    )
    .merge(valid_shapes, on=["route", "id"], how="left")
    .assign(isvalid=lambda x: x.isvalid.fillna(False))
)
...
valhalla_shapes_hi = gpd.read_file(
    os.path.join(path_sp, "data", "01-Interim", "meili_shapes_hi.geojson")
)

valhalla_shapes_data = valhalla_shapes_hi.merge(
    rawnav_agged_out, left_on=["edge_id"], right_on=["id"], how="left"
)

valhalla_shapes_data = valhalla_shapes_data.loc[valhalla_shapes_data.id.notna()]

(
    valhalla_shapes_data.to_file(
