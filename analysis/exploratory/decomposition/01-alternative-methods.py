...
rawnav_stop_window_ind = (
    rawnav_fil[rawnav_fil.stop_window.str.contains("^E")]
...
rawnav_fil2 = (
    ...
    .sort_values(by = ['filename','index_run_start','odom_ft'])
)
...
rawnav_fil4['door_state_changes'] = (
	rawnav_fil4
...
veh_state = (
	...
	.loc[~rawnav_fil4.fps_next.isnull()]
)
...
veh_stop_cases = (
	rawnav_fil5
	.loc[(~rawnav_fil5.veh_state_moving & rawnav_fil5.fps_next.notnull())]
	.groupby(['filename','index_run_start','stop_window_area','veh_state_moving'])
...
rawnav_fil7 = rawnav_fil6
# We start to sort row records into phase based on vars we've created. This is just a first cut.
rawnav_fil7['rough_phase_by_door'] = np.select(
	[
		(rawnav_fil7.door_state_changes < rawnav_fil7.door_state_changes_min), 
		((rawnav_fil7.door_state == "O") 
		  & (rawnav_fil7.door_state_changes == rawnav_fil7.door_state_changes_min)),
		(rawnav_fil7.door_state_changes > rawnav_fil7.door_state_changes_min),
		(rawnav_fil7.door_state_changes_min.isnull()),
	], 
	[
...
rawnav_fil7['rough_phase_by_veh_state'] = np.select(
   [
	(rawnav_fil7.veh_state_changes < rawnav_fil7.veh_stopped_min),
	(rawnav_fil7.veh_state_changes == rawnav_fil7.veh_stopped_min),
	((rawnav_fil7.veh_state_changes > rawnav_fil7.veh_stopped_min)
	 & (rawnav_fil7.veh_state_changes <= rawnav_fil7.veh_stopped_max)),
	(rawnav_fil7.veh_state_changes > rawnav_fil7.veh_stopped_max),
	(rawnav_fil7.veh_stopped_min.isnull())
   ],     
   [
	"t_decel_phase",
...
rawnav_fil7_merge = rawnav_fil7.merge(rawnav_fil_stop_group,
						how='left',
...
rawnav_fil7_merge['at_stop_phase'] = np.select(
	[
		((rawnav_fil7_merge.at_stop) 
		 ...
		 # lead to NA value
			 & (rawnav_fil7_merge.odom_ft_marg == 0)
			 & (rawnav_fil7_merge.rough_phase_by_door == "t_decel_phase")),
		((rawnav_fil7_merge.at_stop) 
			& (rawnav_fil7_merge.odom_ft_marg == 0)
			& (rawnav_fil7_merge.rough_phase_by_door == "t_accel_phase"))
	],
	[
		"t_l_initial",
		"t_l_addl"
