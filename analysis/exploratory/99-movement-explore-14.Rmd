...
most_post_stop_delay3 %>%
  ...
  pull(post)
```
## Distribution of delay at a stop

23697 is Fessenden St NW, not far from Friendship heights
...
fessen_delay <-
  ...
  filter(
    # conditional on serving stop and passengers
    # we could change this up later 
    stop_case == 'atstop' & door_case == "doors"
  ) %>%
  filter(
    relative_to_firstdoor == "post"
  ) %>%
  group_by(
    filename,
    index_run_start,
...
# SMoothing check
How do we make sure that our smoothing isn't too sharp?

Do we still bouts of really sharp acceleration?
