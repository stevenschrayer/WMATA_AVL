...
#        breakpoint()
    ...
    if (rawnav_ti.index.duplicated().any()):
        ...
        assign_statement1 = {
            col_name+'_low': lambda x, thr = threshold: 
                ((x[col_name] < (x[col_name+'_min'] - thr))),
            col_name+'_hi': lambda x, thr = threshold: 
...
def agg_sec(rawnav):
    ...
    rawnav_dupe = (
        ...
        .agg(
            ...
            row_before_apc=('row_before_apc', lambda x: ','.join(
                x.unique().astype(int).astype(str))),
...
def calc_rawnav_speed(rawnav, col_name):
    ...
    rawnav.loc[rawnav.groupby(['filename', 'index_run_start']).tail(
        1).index, col_name+'_speed_next'] = np.nan
...
def calc_rawnav_accel(rawnav, speed_col, groupvars=['filename', 'index_run_start']):
    ...
    assign_statement1 = {
        speed_col+'_accel': lambda x: (x[speed_col] - x[speed_lag_col]) / (x.sec_past_st_next - x.sec_past_st)}
    assign_statement2 = {}
    ...
    rawnav = (
        ...
        .assign(
            ...
            deg_accel_next=lambda x: (x[speed_col] - x[speed_lag_col]) / \
            (x.sec_past_st_next - x.sec_past_st)
        )
