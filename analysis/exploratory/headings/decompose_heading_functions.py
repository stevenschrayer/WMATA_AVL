...
def reset_heading(rawnav):
        ...
        .assign(
            heading_marg=lambda x:
                np.where(
                    ...
                    np.where(
                        x.heading > x.heading_lag,
                        # If the heading is a larger value than the previous one,
                        # the change is actually negative.
                        # Add 360 to the previous value so we get a difference
                        # that falls between -180 and 0.
                        x.heading - (x.heading_lag + 360),
                        # If the heading is a smaller value than the previous one,
                        # the change is actually positive.
                        # Just use modulo. Alternatively you could add 360 to the
                        # difference for the same result.
                        (x.heading - x.heading_lag) % 360
                    )
                )
        )
    ...
    # Cumulatively sum the heading_marg column for a new heading column that doesn't
    # wrap around at 0 and 360
    ...
    rawnav_reset_heading = (
        rawnav_reset_heading
        .assign(
            heading = lambda x: x.heading_new
        )
    )
...
def decompose_heading(rawnav_speed, speed_col, heading_col):
    ...
    heading_class_groups = (
        ...
        .agg(
            heading_start=(heading_col, 'first'),
        ...
        .reset_index()
        .assign(
            heading_chg_deg=lambda x: x.heading_end - x.heading_start,
