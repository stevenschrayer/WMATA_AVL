...
So this looks okay; not sure about that one other_delay bit, but okay. Not sure what to do about that, exactly. It's actually 
```{r, fig.width=6, fig.height=4}
rawnav_decomp %>%
  plot_rawnav_man(.,basic_decomp, odom_min = 17587, odom_max = 18504)
```
...
```{r, fig.width=6, fig.height=4}
plot_rawnav_man(rawnav_decomp,basic_decomp, odom_min = 1500, odom_max = 1630)
```
...
difference between 27 and 28 just doesn't seem that major, especially if not initiated by some deceleration
```{r, fig.width=6, fig.height=4}
plot_rawnav_man(rawnav_decomp2,basic_decomp, odom_min = 6140, odom_max = 6535)
```

this seems to pick up the delay instances we're kind of looking for.

The steady state speed is defined by the pings between the two red dots (something like 20 mph in this instance), but every time the bus hits delay, it gets a little bit slower in the steady state. As a
```{r, fig.width=6, fig.height=4}
...
28069 and 29074 seem like the canonical examples
```{r, fig.width=6, fig.height=4}
rawnav_decomp2 %>%
  plotly::highlight_key(.,~accel_next) %>%
